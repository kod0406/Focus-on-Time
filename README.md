...
### 🎯 개발 동기
컴퓨터로 공부하다 보면 정말 자주 겪는 상황이 있어요. "잠깐만 유튜브 영상 하나만 보고"라고 시작해서 어느새 몇 시간이 훅 지나가버린 경험, 누구나 한 번쯤은 있을 거예요.

저도 개발 공부하면서 이런 일이 너무 많았어요. 브라우저에서 공부하다가 다른 탭으로 넘어가는 건 기본이고, 웹페이지에 있는 광고나 재밌어 보이는 이미지만 봐도 집중력이 흐트러지더라고요. 특히 온라인 강의 듣다가 옆에 떠 있는 추천 영상들 보면... 정말 끝도 없이 빠져들게 되잖아요.

그래서 생각했어요. "아, 이거 근본적으로 막을 방법이 없을까?"

### 💡 프로젝트 아이디어
**"Focus on Time"**은 바로 이런 고민에서 시작된 Chrome 확장 프로그램이에요.

요즘 우리는 정말 많은 정보 속에서 살고 있죠. 특히 컴퓨터로 공부할 때는 온갖 디지털 방해 요소들이 우리 집중력을 방해해요. 그래서 이런 기능들을 만들어봤습니다:

- **🚫 방해 요소를 아예 바꿔버리기**: 집중모드 켜면 웹페이지의 딴짓 유발 이미지들이 다른 이미지로 바뀌어요
...
"집중해!"라고 의지력에만 의존하는 것보다는, 아예 집중할 수밖에 없는 환경을 만들어주는 게 훨씬 효과적이었어요.
...
**참고**: 집중모드는 일반 웹페이지에서만 동작하며, PDF 문서나 로컬 HTML 파일에서는 이미지 변경이 적용되지 않습니다.
